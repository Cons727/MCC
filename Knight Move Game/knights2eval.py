...
class Knights_Move(Game):
    ...
    def __init__(self, N=8):
        ...
        self.initial = GameState(to_move=1, knights={1:knight1, 2:knight2}, moves=set(moves))
...
def eval_fn(state, player):
    ...
    for a in game.actions(state):
        ...
        for b in game.actions(new_state):
            if a == b:
                evaluation += 2

    for action in game.actions(state):
        new_state = game.result(state, action)
        num_moves_i = len(games.__legal_moves(state.moves, new_state.knights[player]))
        if num_moves_i > num_moves:
            num_moves = num_moves_i
    evaluation += 1.4 * num_moves
...
def alphabeta_player(depth=2, eval_fn=eval_fn):
    ...
    return (lambda game, state: alphabeta_search(state, game, d=depth, eval_fn=eval_fn))
